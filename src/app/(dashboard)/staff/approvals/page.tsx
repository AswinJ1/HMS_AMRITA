...
export default function StaffApprovalsPage() {
  ...
  const pending = applyFilters(
    approvals.filter((a) => a.status === "PENDING" && a.request?.stage === "STAFF_PENDING")
  )
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Review stayback requests at the Staff approval stage.</p>
          </div>
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>Requests awaiting your action (Staff stage)</CardDescription>
          </CardHeader>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div>
                ...
                <div className="mt-1 flex items-center justify-between border p-2">
                  ...
                  <div className="text-right">
                    <Badge
                      variant={selected?.request?.securityStatus === "IN" ? "default" : selected?.request?.securityStatus === "OUT" ? "secondary" : "outline"}
                      className="text-[10px]"
                    >
          ...
          <DialogContent className="max-w-md">
            ...
            {viewItem && (
              <div className="space-y-3 text-sm">
                ...
                {/* Prior Approvals */}
                {viewItem.request?.approvals && viewItem.request.approvals.filter((ap: any) => ap.teamLead).length > 0 && (
                  <div>
                    <p className="text-[10px] font-semibold uppercase text-muted-foreground">Prior Approvals</p>
                    {viewItem.request.approvals.filter((ap: any) => ap.teamLead).map((ap: any) => (
                      <div key={ap.id} className="mt-1 flex items-center justify-between border p-2">
                        <span className="text-xs">{ap.teamLead.name} (TL)</span>
                        <div className="text-right">
                          <Badge variant={ap.status === "APPROVED" ? "default" : "secondary"} className="text-[10px]">{ap.status}</Badge>
