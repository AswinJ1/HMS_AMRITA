...
export default function StaffApprovalsPage() {
  ...
  const pending = approvals.filter((a) => a.status === "PENDING" && a.request?.stage === "STAFF_PENDING")
  const history = approvals.filter((a) => a.status !== "PENDING" || a.request?.stage !== "STAFF_PENDING")
