...
export default function StaffDashboard() {
  ...
  const pending = approvals.filter((a) => a.status === "PENDING" && a.request?.stage === "STAFF_PENDING").length
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button size="sm" asChild>
            <Link href="/staff/approvals"><ClipboardCheck className="mr-2 size-4" /> Review Approvals</Link>
          </Button>
        </div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            <div>
              ...
              <CardDescription>Requests at Staff stage awaiting your review</CardDescription>
            </div>
            <Button variant="ghost" size="sm" asChild>
              <Link href="/staff/approvals">View All <ArrowRight className="ml-1 size-3.5" /></Link>
            </Button>
          </CardHeader>
          <CardContent className="px-4 pb-4">
            {loading ? (
              ...
              <div className="space-y-2">
                {approvals
                  .filter((a) => a.status === "PENDING" && a.request?.stage === "STAFF_PENDING")
                  .slice(0, 5)
                  .map((a) => {
