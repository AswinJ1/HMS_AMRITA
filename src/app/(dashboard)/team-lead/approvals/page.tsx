...
export default function TeamLeadApprovalsPage() {
  ...
  const pending = approvals.filter((a) => a.status === "PENDING" && a.request?.stage === "TEAM_LEAD_PENDING")
  const history = approvals.filter((a) => a.status !== "PENDING" || a.request?.stage !== "TEAM_LEAD_PENDING")
